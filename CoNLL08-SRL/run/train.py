	...
	while global_step < config.train_iters:
		...
		for words, lemmas, tags, arcs, rels in \
			...
			if global_step % config.validate_every == 0:
				...
				NF1, F1 = test(parser, vocab, config.num_buckets_valid, config.test_batch_size, config.unlabeled_dev_file, 
								config.labeled_dev_file, config.raw_dev_file, os.path.join(config.save_dir, 'valid_tmp'))
				history(NF1, F1)
